...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    setIsloading(true);
    fetch("http://172.20.10.5:3000/api-search/products")
  ...
  const getProductsByCategory = (category, index) => {
    ...
    if (index != 0) {
      const url = `http://172.20.10.5:3000/api/product/getByCat/${category}`;
      ...
      fetch("http://172.20.10.5/api-search/products")
        .then((response) => response.json())
        .then((json) => setProduct(json))
        .catch((error) => console.error(error))
        .finally(() => setIsloading(false));
